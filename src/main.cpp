...
static bool waitForResponse(const char *expect, uint32_t timeoutMs) {
  ...
  while (millis() - start < timeoutMs) {
    while (MODEM.available()) {
      ...
      Serial.write(c); // mirror to USB for visibility
      buf += c;
    ...
    if (ch == '\r' || ch == '\n') {
      if (cmd.length() > 0) {
        if (cmd.startsWith("!help")) {
          Serial.println(F("Commands: !test, !apn <name>, !netopen, !close, !ntrip <host> <mount> <user> <pass>, !tcp <host> <port>, !basic, !post <json>, !httppost <host> <port> <path> <json>, !fpost <url_path> <json>, !rtdb <path> <json> [auth], !cap"));
